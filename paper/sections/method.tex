...
The first two---normalization and single-term search---are the core
technical contributions; the remaining three provide enabling context.

\paragraph{Normalization resolves gauge freedom (key contribution).}
The parameterization $c \cdot g(ax + b)$ introduces redundancies.
For $g = \exp$, the identity $c \cdot \exp(ax + b) = (c\,e^{b}) \cdot \exp(ax)$
means that $c$ and $b$ are not independently identifiable: the optimizer
can distribute scale information between $c$ and $e^{b}$ arbitrarily,
creating a ridge in the loss landscape along which it may drift.
For $g = \sin$ or $g = \cos$, the periodicity and parity identities
create analogous degeneracies---$\sin(x - \pi) = -\sin(x)$ and
$\sin(x - \pi/2) = -\cos(x)$, so a $\sin$ factor with a phase offset
of $-\pi/2$ is indistinguishable from a $\cos$ factor with a negated
coefficient.  The normalization step
...
has a unique interpretation.  This is the most impactful design choice:
without normalization, snapping errors are $0.005$--$0.02$; with it,
errors drop to $7 \times 10^{-8}$---a five-order-of-magnitude improvement
(Section~\ref{sec:ablation}).

\paragraph{Single-term search avoids signal splitting (key contribution).}
...
\paragraph{EML provides a principled basis.}
The choice of primitive library $\mathcal{F} = \{\sin, \cos, \exp, \ln,
\mathrm{id}, \mathrm{sq}, \mathrm{inv}\}$ is not \emph{ad hoc}.
Every element of $\mathcal{F}$ admits a verified construction from the
universal EML function $\mathrm{eml}(x, y) = e^{x} - \ln y$
(Section~\ref{sec:primitives}).  The universality theorem
of~\cite{odrzywołek2026} guarantees that this basis can, in principle,
generate any computable function through further composition.
More practically, the EML function class is closed under partial
differentiation (Appendix~\ref{appendix:algebraic-proofs}), which ensures
that PDE residual computations---requiring $\partial_{t} u$, $\partial_{x} u$,
$\partial_{xx} u$, etc.---remain within the same function class.
This closure property is what enables physics-informed training through
the compositional graph.

\paragraph{Gradient-based optimization.}
Every operation in our pipeline is differentiable.  Each EML
primitive---$\exp$ via $\mathrm{eml}(x, 1)$, $\sin$ and $\cos$ via the
imaginary and real parts of $\mathrm{eml}(ix, 1) = e^{ix}$---supports
standard backpropagation.  This allows us to leverage mature gradient-based
optimizers (Adam) with well-understood convergence properties, rather
than relying on stochastic search heuristics.  The per-candidate
optimization problem is low-dimensional ($2K + 1$ parameters), smooth,
and---for the correct function assignment---typically convex in the
