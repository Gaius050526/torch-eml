...
Preliminary experiments show that raw EML trees can partially recover $\tanh$
(MAE $\sim 10^{-2}$) without being told the function exists, but the
optimization landscape for deep compositions is orders of magnitude harder than
for named primitives---confirming that ``naming a function'' is not merely a
notational convenience but a critical compression that makes optimization
tractable.  Developing better optimization methods for deep EML trees---evolutionary
strategies, hierarchical motif extraction, or reinforcement learning---is a
central open problem that could transform EML from a tool for recovering known
solutions into a tool for discovering new mathematics.
